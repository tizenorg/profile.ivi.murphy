...
generate_changelog() {
    local _version="$1" _gitver="$2" _author="$3"
    local _sha

    _sha="`git rev-parse $_gitver`"

    echo "* $(date '+%a %b %d %H:%M:%S %Z %Y') $_author - $_version" && \
    echo "- release: released $_version (git: $_sha)."
}
...
# prepare for gbs
gbs_prepare() {
    local _specin _specout _chlog _no_patches
    ...
    generate_specfile $_specin $_specout $VERSION $_no_patches && \
        ...
        generate_changelog $VERSION $HEAD "$AUTHOR" > $_chlog && \
