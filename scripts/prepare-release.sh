...
generate_patches() {
    ...
    info "* Generating patches from $_baseline till $_head..."
    pushd $_dir >& /dev/null && \
            ...
            --version|-v)
             ...
             --gbs*)
                if [ -z "$MODE" ]; then
                    ...
                    error "Multiple mode options ($1, $MODE) specified."
                    usage
                fi
                ;;
             --gerrit*|-G)
...
gbs_prepare() {
    ...
    if [ -z "$BASELINE" ]; then
        ...
        _base=$BASELINE
        generate_patches $_base $RELEASE $_dir
        _patches="`ls $_dir | grep '.patch$'`"
        echo "$_patches" | tr -s '\t' ' ' | tr ' ' '\n' | \
            indent " PATCH: "
    fi

    generate_specfile $_specin $_specout $VERSION $_patches | indent "  SPEC: "
    generate_branch $_branch $_base | indent "BRANCH: "
    run_gbs_quirks 2>&1 | indent "  PREP: "
    generate_changelog $_chlog $VERSION $RELEASE "$AUTHOR" | indent "CHGLOG: "
...
# export to OBS
obs_export() {
    local _dir _specin _specout _patches
    ...
    if [ -n "$BASELINE" ]; then
        _base=$BASELINE
        generate_patches $_base $RELEASE $_dir | indent " PATCH: "
        _patches="`ls $_dir | grep '.patch$'`"
        echo "$_patches" | tr -s '\t' ' ' | tr ' ' '\n' | \
            indent "PATCHES: "
    else
    ...
    generate_specfile $_specin $_specout $VERSION $_patches | indent " SPEC: "
    generate_tarball $PKG $VERSION $_base $_dir | indent "  TAR: "
}
...
[ -z "$MODE" ]    && MODE=gbs
[ -z "$RELEASE" ] && RELEASE=$(git_current_branch)
[ -z "$VERSION" ] && VERSION=$(default_version)
[ -z "$TARGET" ]  && TARGET="2.0alpha"
[ -z "$AUTHOR" ]  && AUTHOR="`git config user.name` <`git config \
                                  user.email`>"

show_config | indent " CONFIG: "
sleep 2

if [ "$MODE" = "gbs" -a -n "$BASELINE" ]; then
    echo "* Forcing baseline off in gerrit mode"
    BASELINE=""
fi
